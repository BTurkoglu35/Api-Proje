...
Then
   ...
   */
    ...
    public void name() {
        ...
        //Groovy language =java tememlli programlama dili
        System.out.println("Id'si 190 dan buyuk olanlar : "+idler);

        //Assert that there are 10 ids greater than 190=> 10 tane id nin 190 dan buyuk olani dogrulayin
        assertEquals("id 190 dan buyuk olan eslesmedi",10,idler.size());

       //3)Print all userIds whose ids are less than 5 on the console ==> id si 5 den kucuk olan tum userid lerini konsolunu yazdirin
       List<Integer> userIdBesdenKucuk=json.getList("findAll{it.id<5}.userId");
        System.out.println("id' si 5 den kuuck olan user id'ler : "+userIdBesdenKucuk);
     //   Assert that the number of userIds whose ids are less than 5 is 4 ==> id si 5 den kucuk olan 4 tane userId oldugunu dogrulayin
     assertEquals("id'si 5 den kucuk olan user idler 4 tane degil",4,userIdBesdenKucuk.size());
   ...
   //     Assert that "delectus aut autem" is one of the titles whose id is less than 5 ==> id si 5 den kucuk olan datalarin birinin
   //     basliginin "delectus aut autem" icerdigini dogrulayin
    assertTrue("titler dan bir tanesi delectus autem icermemektedir.",titles.contains("delectus aut autem"));

        assertTrue("titler dan bir tanesi delectus autem icermemektedir.",
